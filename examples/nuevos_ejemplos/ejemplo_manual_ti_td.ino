...
// Parámetros del PID (Kp, Tiempo Integral Ti, Tiempo Derivativo Td)
// Estos valores se deben ajustar/sintonizar para cada proceso
double Kp = 2.0;  // Ganancia Proporcional
double Ti = 1.0;  // Tiempo Integral (segundos)
double Td = 0.05; // Tiempo Derivativo (segundos)
...
void loop() {
  ...
  if (millis() - tiempo_anterior >= (unsigned long)(dt * 1000)) {
    ...
    // PID_kernelOS(controlador, tiempo_actual_sec, Kp, Ti, Td)
    // Se usa la variante con Kp, Ti (Tiempo Integral en segundos), Td (Tiempo Derivativo en segundos)
    PID_kernelOS(pid_controller, millis() / 1000.0, Kp, Ti, Td);
